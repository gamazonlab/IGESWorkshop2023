...
The following tutorial is designed to give an overview of data analyses using single cell sequencing datasets using the Seurat procedure. 
...
N.B.For this workshop, we have subsetted the control(C51ctr,C53ctr,C56ctr) & cases(L11cov, L22cov, L07cov) samples. 
Please download the smaller dataset and metadata from: https://github.com/gamazonlab/IGESWorkshop2023.git
Once downloaded onto local computer, create a folder, move the files to the folder andunzip them. 
...
One approach after clustering is to perform differential expression analysis.
...
At this point, there is no inbuilt cell type labeling in Seurat, so this step is done manually.
...
In this case, the goal is to identify those genes that are differentially expression between two experimental conditions such as (stimulated vs unstimulated cells) or between disease cases and healthy controls in specific cell type. However, having only 3 patients is probably too low, with many more patients it will work better to run pseudobulk analysis.
...
❗️It happens here, that many of the genes are evenly expressed across samples. In the case that DEG results are dominated by single sample, one approach is to downsample Seurat function to make sure that every sample has the same number of cells.
